...
export type AuditEventType =
  ...
  | "pipes_authority.request_approved"
  | "pipes_authority.request_denied"
  | "api_call.completed"
  | "api_call.denied"
  | "api_call.errored"
  | "api_call.per_request_approved";
...
function buildMetadata(
  ...
  if (event.approvalId) m.approvalId = event.approvalId;
  return m;
}
...
export async function registerAuditLogSchemas(): Promise<void> {
  ...
  const schemas: {
    ...
    {
      action: "pipes_mcp.pipes_authority.request_approved",
      targets: [targetTypes.session],
      metadata: metadataSchema,
    },
    {
      action: "pipes_mcp.pipes_authority.request_denied",
      targets: [targetTypes.session],
      metadata: metadataSchema,
    },
    {
