...
import { getOrCreateSession, getSessionAuthority } from "./session";
...
function getScopesForAuthority(authority: "none" | "read" | "write"): string[] {
  if (authority === "write") {
...
export const verifyToken = async (
  ...
  try {
    ...
    // No fallback — if the JWT lacks org_id, the session is org-less.
    ...
    return {
      token: bearerToken,
      scopes: getScopesForAuthority(getSessionAuthority(session)),
      clientId: user.id,
