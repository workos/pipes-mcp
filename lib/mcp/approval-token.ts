...
export interface ApprovalTokenPayload {
  ...
  reason?: string;
  /** Per-request approval: the specific API call being requested (authority="request" only) */
  requestDetails?: {
    url: string;
    method: string;
  };
...
export async function createApprovalToken(
  ...
  const jwe = await new jose.EncryptJWT({
    ...
    .setIssuedAt()
    .setIssuer(ISSUER)
    .setJti(payload.tokenId ?? crypto.randomUUID())
...
> {
  ...
  return {
    ...
    authority: payload.authority as "read" | "write" | "request",
    integrations:
    ...
    requestDetails: payload.requestDetails as
